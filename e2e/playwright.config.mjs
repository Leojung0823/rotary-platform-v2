  ...
  projects: [
    {
      ...
      testIgnore: /role-shells.*\.e2e\.mjs|event-create-form\.e2e\.mjs|member-home.*\.e2e\.mjs|dynamic-checkin.*\.e2e\.mjs|location-checkin\.e2e\.mjs|mobile-nav-contrast\.e2e\.mjs|event-cover\.e2e\.mjs|birthday-collection\.e2e\.mjs|birthday-v2\.e2e\.mjs|line-rich-menu\.e2e\.mjs/,
    },
    {
    ...
    ...[1440, 375].map((width) => ({
      name: `event-detail-${width}`,
      testMatch: eventDetailTestMatch,
      use: width <= 412
        ? { viewport: { width, height: 812 }, isMobile: true, hasTouch: true, deviceScaleFactor: 2 }
        : { viewport: { width, height: 900 } },
    })),
    {
      name: "board-audience-1440",
    ...
    ...[1440, 375].map((width) => ({
      ...
      testMatch: memberTagsTestMatch,
      use: width <= 412
        ? { viewport: { width, height: 812 }, isMobile: true, hasTouch: true, deviceScaleFactor: 2 }
        : { viewport: { width, height: 900 } },
    })),
    ...[1440, 375].map((width) => ({
      name: `blessing-privacy-${width}`,
      testMatch: blessingPrivacyTestMatch,
      use: width <= 412
        ? { viewport: { width, height: 812 }, isMobile: true, hasTouch: true, deviceScaleFactor: 2 }
        : { viewport: { width, height: 900 } },
    })),
    ...[1440, 375].map((width) => ({
      name: `interact-hub-${width}`,
      testMatch: interactHubTestMatch,
      use: width <= 412
        ? { viewport: { width, height: 812 }, isMobile: true, hasTouch: true, deviceScaleFactor: 2 }
        : { viewport: { width, height: 900 } },
    })),
    ...
    ...[1440, 412].map((width) => ({
      name: `attendance-${width}`,
      testMatch: attendanceTestMatch,
      use: width <= 412
