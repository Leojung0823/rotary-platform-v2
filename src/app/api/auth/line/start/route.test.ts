...
describe("GET /api/auth/line/start", () => {
  ...
  it("persists only state and nonce digests with required metadata", async () => {
    await route.GET(new NextRequest("http://localhost:3000/api/auth/line/start?returnTo=/join&invite=opaque-invite"));
    ...
    expect(inserted.state_hash).not.toBe(stateCookie);
    expect(inserted.invitation_token_hash).toHaveLength(64);
    expect(inserted.return_path).toBe("/join");
  });

  it("sets state and nonce cookies with the same secure attributes and TTL", async () => {
    await route.GET(new NextRequest("http://localhost:3000/api/auth/line/start"));
    const stateCall = mocks.cookieSet.mock.calls.find((call) => call[0] === "line_oauth_state")!;
    const nonceCall = mocks.cookieSet.mock.calls.find((call) => call[0] === "line_oauth_nonce")!;
    expect(stateCall[2]).toEqual(nonceCall[2]);
    expect(stateCall[2]).toEqual({ httpOnly: true, secure: false, sameSite: "lax", path: "/", maxAge: 600 });
  });

  it("does not redirect to the provider when state persistence fails", async () => {
  ...
  it("rejects production mock mode before state persistence", async () => {
