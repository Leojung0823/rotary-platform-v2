...
export default async function OperatorsPage({ params, searchParams }: { params: Promise<{ clubId: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  <section><div className="section-heading"><h2>邀請紀錄</h2></div><div className="table-wrap"><table><thead><tr><th>受邀者</th><th>電子郵件</th><th>狀態</th><th>到期時間</th></tr></thead><tbody>{management.invitations.map(invite => <tr key={invite.invite_id}><td>{invite.display_name}</td><td>{invite.email}</td><td><Badge tone={invite.invite_status === "accepted" ? "success" : invite.invite_status === "sent" ? "warning" : "neutral"}>{statusLabel(invite.invite_status)}</Badge></td><td>{new Intl.DateTimeFormat("zh-TW", { timeZone: APP_TIME_ZONE,  dateStyle: "medium", timeStyle: "short" }).format(new Date(invite.expires_at))}</td></tr>)}</tbody></table></div></section></div>;
}
