...
export default async function OperatorsPage({ params, searchParams }: { params: Promise<{ clubId: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const successMessages: Record<string, string> = { invited: "新邀請已寄至本機 Mailpit。", accepted: "邀請已接受，扶輪社管理權限已啟用。", revoked: "執行秘書權限已撤銷，歷史紀錄已保留。" };
  ...
  <Card><h2>邀請執行秘書</h2><p className="subtle">有效社友不能同時擔任執行秘書。邀請只會寄到本機 Mailpit。</p><form action={inviteOperatorAction} className="inline-form"><input type="hidden" name="clubId" value={clubId}/><Field label="姓名"><Input name="displayName" required /></Field><Field label="電子郵件"><Input name="email" type="email" required /></Field><Button type="submit">寄出邀請</Button></form></Card>
