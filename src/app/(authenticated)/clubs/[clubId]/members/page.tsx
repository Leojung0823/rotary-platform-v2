...
import { MemberTagBatchForm } from "@/components/members/member-tag-batch-form";
...
// Three states, not two: a member who added the OA but is not linked to a
// membership yet used to read as 「未加入」, so the club asked them to add an
// OA they had already added. What they need is one click on the LINE OA page.
const oaLabels: Record<string, string> = {
  following: "已加入",
  awaiting_pairing: "已加入，待配對",
  blocked: "已封鎖",
  unpaired: "未加入",
};

function oaTone(status: string) {
  if (status === "following") return "success" as const;
  if (status === "awaiting_pairing") return "warning" as const;
...
const tagMessages: Record<string, string> = {
  ...
  tags_saved: "社員標籤已更新。",
  tags_batch_assigned: "已貼上標籤。",
  tags_batch_removed: "已移除標籤。",
  no_members_selected: "請先勾選要貼標籤的社員。",
  member_missing: "選取的社員中有人已不屬於本社，請重新整理後再試。",
  tag_exists: "同名標籤已存在。",
...
export default async function MembersPage({ params, searchParams }: { params: Promise<{ clubId: string }>; searchParams: Promise<{ q?: string; status?: string; success?: string; error?: string; count?: string }> }) {
  ...
  // Only the batch actions report a count, and it is the number that actually
  // changed rather than the number selected -- re-tagging someone who already
  // had the tag is a no-op the officer should not read as a failure.
  const parsedCount = Number.parseInt(query.count ?? "", 10);
  const batchCount = Number.isNaN(parsedCount) ? null : parsedCount;
  return <div className="page-stack"><header className="page-header"><div><p className="eyebrow">會員管理</p><h1>社員</h1><p>先由扶輪社建立已知資料，再邀請社員完成身份確認。</p></div><div className="form-actions"><Link className="button button-secondary" href={`/clubs/${clubId}/members/archive?mode=management`} prefetch={false}>封存社員</Link><a className="button button-secondary" href={`/api/v1/clubs/${clubId}/members/export`}>Excel 匯出</a><Link className="button" href={`/clubs/${clubId}/members/new?mode=management`} prefetch={false}>＋ 新增社員</Link></div></header><ClubAdminNav clubId={clubId}/>{query.success && <Notice tone="success">{tagMessages[query.success] ?? "社員狀態已更新。"}{batchCount !== null && ` 共 ${batchCount} 位社員異動。`}</Notice>}{query.error && <Notice tone="error">{tagMessages[query.error] ?? tagMessages.unexpected}</Notice>}<Card id="member-tags"><div className="section-heading"><div><p className="eyebrow">分眾</p><h2>社員標籤</h2></div><span>{tags.length} 個標籤</span></div><p>標籤用來指定活動與訊息的對象，例如理事會、新社員。設定了對象的活動不是例會，不會計入出席。</p><form action={createMemberTagAction} className="inline-form"><input type="hidden" name="clubId" value={clubId}/><Field label="標籤名稱"><Input name="tagName" required maxLength={40} placeholder="例如：理事會"/></Field><Field label="說明（選填）"><Input name="description" maxLength={200} placeholder="例如：理事與監事"/></Field><Button type="submit">建立標籤</Button></form>{tags.length === 0 ? <p className="subtle">尚未建立任何標籤。</p> : <div className="table-wrap"><table><thead><tr><th>標籤</th><th>說明</th><th>人數</th><th></th></tr></thead><tbody>{tags.map(tag => <tr key={tag.tag_id}><td><strong>{tag.tag_name}</strong></td><td>{tag.description ?? "—"}</td><td>{tag.member_count}</td><td><form action={archiveMemberTagAction}><input type="hidden" name="clubId" value={clubId}/><input type="hidden" name="tagId" value={tag.tag_id}/><Button className="button-secondary" type="submit">封存</Button></form></td></tr>)}</tbody></table></div>}</Card><form className="inline-form"><input type="hidden" name="mode" value="management"/><Input name="q" defaultValue={query.q} placeholder="搜尋姓名、手機或 Email"/><Select name="status" defaultValue={query.status ?? ""}><option value="">全部狀態</option><option value="active">啟用</option><option value="invited">受邀中</option><option value="suspended">暫停</option><option value="disabled">停用（在封存頁）</option></Select><button className="button" type="submit">搜尋</button></form>{members.length === 0 ? <EmptyState title="找不到社員" body="調整搜尋條件，或新增第一位社員。"/> : <MemberTagBatchForm clubId={clubId} tags={tags.map(tag => ({ tag_id: tag.tag_id, tag_name: tag.tag_name }))}><div className="table-wrap"><table><thead><tr><th><span className="sr-only">選取</span></th><th>社員</th><th>聯絡方式</th><th>社籍</th><th>LINE Login</th><th>LINE OA</th><th></th></tr></thead><tbody>{members.map(member => <tr key={member.membership_id}><td><input aria-label={`選取 ${member.display_name}`} name="membershipIds" type="checkbox" value={member.membership_id}/></td><td><strong>{member.display_name}</strong></td><td>{member.phone ?? member.email ?? "—"}</td><td><Badge tone={member.membership_status === "active" ? "success" : "warning"}>{membershipLabel[member.membership_status] ?? member.membership_status}</Badge></td><td><Badge tone={member.line_identity_id ? "success" : "neutral"}>{member.line_identity_id ? "已綁定" : "未綁定"}</Badge></td><td><Badge tone={oaTone(member.oa_status)}>{oaLabels[member.oa_status] ?? "未加入"}</Badge>{member.oa_status === "awaiting_pairing" && <><br /><Link className="subtle" href={`/clubs/${clubId}/line-oa?mode=management`} prefetch={false}>去配對 →</Link></>}</td><td><Link href={`/clubs/${clubId}/members/${member.membership_id}?mode=management`} prefetch={false}>管理 →</Link></td></tr>)}</tbody></table></div></MemberTagBatchForm>}</div>;
}
