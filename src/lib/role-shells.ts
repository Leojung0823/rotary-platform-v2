...
const navigationByMode: Readonly<Record<ExperienceMode, readonly NavigationDefinition[]>> = {
  ...
  // count is announced. 互動 and 社務 were kept out while this bar was held to
  // four; both turned out to be places members go often enough to deserve a tab,
  // so they were promoted (2026-09-14). 社務 is the member's view of how the
  // club is run -- club details, officers, the year's service plan, the
  // document centre -- and is not a way into management mode, which still lives
  // in the account menu. None of these links is an authorization boundary:
  // every destination still checks its own route, RPC and RLS authority.
  member: [
...
export function resolveCurrentNavigationItemId(
  ...
  if (managementOverview && /^\/clubs\/[^/]+\/(?:archives|birthday-collection|blessing-iou|dues|identity|invitations|operators|service-plan)(?:\/|$)/u.test(currentPath)) {
