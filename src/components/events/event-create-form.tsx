...
export function EventCreateForm({ clubId, eventTypeLabels }: EventCreateFormProps) {
  ...
  const [draftValues, setDraftValues] = useState<EventCreateFormValues>(initialEventCreateFormValues);
  const [editedRevision, setEditedRevision] = useState(initialEventCreateActionState.revision);
  const errorSummaryRef = useRef<HTMLDivElement>(null);
  const showingActionValues = state.status === "error" && editedRevision !== state.revision;
  const values = showingActionValues ? state.values : draftValues;

  const updateValues = (update: Partial<EventCreateFormValues>) => {
    setDraftValues((current) => ({ ...(showingActionValues ? state.values : current), ...update }));
    if (showingActionValues) setEditedRevision(state.revision);
  };
  ...
  return <form action={formAction} className="form-stack" noValidate>
    ...
    <div className="form-grid">
      <label className="field" htmlFor="event-create-eventType"><span className="label">活動類型</span>
        <select
          ...
          onChange={(event) => updateValues({ eventType: event.target.value })}
      ...
      <label className="field" htmlFor="event-create-title"><span className="label">活動名稱</span>
        <input className="input" id="event-create-title" name="title" maxLength={160} required value={values.title} onChange={(event) => updateValues({ title: event.target.value })} aria-invalid={Boolean(errorFor("title"))} aria-describedby={describedBy("title")} />
      ...
      <label className="field" htmlFor="event-create-startsAt"><span className="label">開始時間（台北）</span>
        <input className="input" id="event-create-startsAt" type="datetime-local" name="startsAt" required value={values.startsAt} onChange={(event) => updateValues({ startsAt: event.target.value })} aria-invalid={Boolean(errorFor("startsAt"))} aria-describedby={describedBy("startsAt")} />
      ...
      <label className="field" htmlFor="event-create-endsAt"><span className="label">結束時間（台北）</span>
        <input className="input" id="event-create-endsAt" type="datetime-local" name="endsAt" required value={values.endsAt} onChange={(event) => updateValues({ endsAt: event.target.value })} aria-invalid={Boolean(errorFor("endsAt"))} aria-describedby={describedBy("endsAt")} />
      ...
      <label className="field" htmlFor="event-create-registrationDeadline"><span className="label">報名截止（台北）</span>
        <input className="input" id="event-create-registrationDeadline" type="datetime-local" name="registrationDeadline" required value={values.registrationDeadline} onChange={(event) => updateValues({ registrationDeadline: event.target.value })} aria-invalid={Boolean(errorFor("registrationDeadline"))} aria-describedby={describedBy("registrationDeadline")} />
      ...
      <label className="field" htmlFor="event-create-capacity"><span className="label">名額（留空表示不限）</span>
        <input className="input" id="event-create-capacity" type="number" name="capacity" min={1} max={10000} inputMode="numeric" value={values.capacity} onChange={(event) => updateValues({ capacity: event.target.value })} aria-invalid={Boolean(errorFor("capacity"))} aria-describedby={describedBy("capacity")} />
      ...
      <label className="field" htmlFor="event-create-location"><span className="label">地點</span>
        <input className="input" id="event-create-location" name="location" maxLength={300} value={values.location} onChange={(event) => updateValues({ location: event.target.value })} aria-invalid={Boolean(errorFor("location"))} aria-describedby={describedBy("location")} />
      ...
      <label className="checkbox-row" htmlFor="event-create-countsForAttendance">
        <input id="event-create-countsForAttendance" type="checkbox" name="countsForAttendance" checked={values.countsForAttendance} onChange={(event) => updateValues({ countsForAttendance: event.target.checked })} aria-invalid={Boolean(errorFor("countsForAttendance"))} aria-describedby={describedBy("countsForAttendance")} />
    ...
    <label className="field" htmlFor="event-create-description"><span className="label">活動說明</span>
      <textarea className="input" id="event-create-description" name="description" maxLength={5000} rows={4} value={values.description} onChange={(event) => updateValues({ description: event.target.value })} aria-invalid={Boolean(errorFor("description"))} aria-describedby={describedBy("description")} />
