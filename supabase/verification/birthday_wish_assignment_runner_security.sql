...
begin
  ...
  if (select count(*) from public.birthday_wish_campaign_participants where assignment_batch_id = batch_id) <> 2
     ...
     or (select count(distinct question_bank_item_id) from public.birthday_wish_campaign_participants where assignment_batch_id = batch_id) <> 2
     or (select count(distinct lower(btrim(question_prompt_snapshot))) from public.birthday_wish_campaign_participants where assignment_batch_id = batch_id) <> 2 then
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'birthday_wish_campaign_participants'
      and indexname = 'birthday_participant_batch_prompt_unique'
  ) then
    raise exception 'birthday prompt snapshot uniqueness index is missing';
  end if;
end $$;
