...
-- The homepage is an action list, not the message-centre archive. A birthday
-- invitation remains useful there while it is pending or needs a new
-- submission, but it must leave the list after the member submits it. The
-- message-centre history still keeps the original delivery row.
