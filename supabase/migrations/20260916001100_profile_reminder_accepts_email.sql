...
-- 一個清不掉的提醒。
--
-- #179 加了「個人資料未完成」，而它問的是「有沒有填手機」。/me 上寫著的規則
-- 是「手機與 Email 至少保留一項」，update_my_profile 也正是這樣擋的 -- 所以
-- 一位只留 Email 的社友去把資料填好、平台說他合格，提醒隔天還在，而且沒有
-- 任何操作可以讓它消失。
--
-- 那支 migration 自己的註解寫的是 a way to reach the member；意圖一直是
-- 「聯絡得到嗎」，只有實作把它窄成了電話。這裡讓兩者一致。
--
-- Restated from the current definition; only profile_gaps and the one branch
-- that reads it change.
