...
-- Invalid-signature payloads are retained for security telemetry, but they must not
-- reserve a provider event id and suppress a later valid delivery.
drop index if exists public.line_webhooks_provider_event_unique;
create unique index line_webhooks_valid_provider_event_unique
  on public.line_webhooks (line_oa_account_id, provider_event_id)
  where provider_event_id is not null and signature_valid;

-- Sanitized club codes can collide (for example A-B and A_B). Refuse to activate
-- two OA configurations that resolve to the same server-side credential namespace.
create unique index line_oa_accounts_active_webhook_env_unique
  on public.line_oa_accounts (webhook_secret_env_key)
  where account_status <> 'disabled';
create unique index line_oa_accounts_active_access_token_env_unique
  on public.line_oa_accounts (access_token_env_key)
  where account_status <> 'disabled';
...
create or replace function public.pair_line_oa_follower(
  p_club_id uuid,
  p_oa_user_id text,
  p_person_id uuid
)
...
begin
  ...
  if btrim(coalesce(p_oa_user_id, '')) !~ '^U[A-Za-z0-9_-]{8,128}$' then
    raise exception using errcode = '22023', message = 'invalid_oa_user_id';
  end if;
  ...
  if oa_id is null
    ...
    raise exception using errcode = 'P0002', message = 'active_oa_member_not_found';
  end if;

  select id into target_account
  from public.app_accounts
  where person_id = p_person_id and account_status = 'active';

  insert into public.line_oa_followers (
    line_oa_account_id, club_id, person_id, app_account_id, oa_user_id, paired_at
  ) values (
    oa_id, p_club_id, p_person_id, target_account, btrim(p_oa_user_id), now()
  )
  on conflict (line_oa_account_id, oa_user_id)
  do update set
  ...
  insert into public.audit_logs (
    ...
    p_club_id, actor_id, 'line_oa.paired', 'line_oa_follower', follower_id
  );
