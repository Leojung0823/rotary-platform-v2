...
-- The member home gains the one thing its "待辦提醒" column needs: the events
-- this member has been asked about and has not answered.
--
-- Everything required is already in candidate_events, which is published,
-- unfinished events for this club with the member's own response attached, so
-- this adds a filter over it rather than another pass over the tables. No
-- response and an explicitly pending response are treated alike: in both cases
-- nobody knows whether the member is coming.
--
-- The row carries hours remaining rather than a label. How soon "soon" is, and
-- what to call it, is a product decision that belongs to the page and can
-- change without a migration.
--
-- Restated from the current definition; nothing else about this function
-- changes.
