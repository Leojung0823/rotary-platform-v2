...
-- The home's "近期活動" column needs more than the two events the projection
-- returns for the hero. It reuses ranked_events -- the ordering the hero
-- already uses -- rather than ranking again, so the first row of the list and
-- the hero can never disagree about which event is next.
--
-- Each row carries a registration state drawn the same way the hero draws it,
-- for the same reason: one event must not read as two different things in two
-- places on one page.
--
-- Restated from the current definition; nothing else changes.
