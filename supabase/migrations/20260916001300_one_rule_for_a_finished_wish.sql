...
-- 社員寫完了，提醒還在。
--
-- 生日祝福的待辦問的是「這則祝福發布了沒有」。發布是幹部的動作 -- 社員把
-- 祝福寫好送出之後，就沒有任何事情是他做得到的了，而「生日祝福待填寫」還
-- 掛在他的首頁上。
--
-- 更精確地說：同一支函式裡，同一個問題有兩條規則。十行之下的社內訊息，用
-- 同樣那幾個狀態算出來的答案是 completed -- 也就是說，同一位社員的同一則
-- 指派，訊息說做完了、待辦說還沒有。
--
-- 把規則收成一支函式，兩邊都問它。這和 event_registration_is_open 當初的
-- 處理方式一樣：重複的規則遲早會各自長歪，而它們長歪的時候沒有東西會響。
--
-- 行為上唯一的改變是待辦那半：submitted 從「還沒做」變成「做完了」，跟訊息
-- 那半本來就有的答案一致。hidden（幹部退回重寫）仍然是社員的事。

create or replace function public.birthday_wish_action_status(
...
as $$
  select case
    when p_participant_status = 'declined' then 'declined'
    when p_participant_status = 'disabled' then 'disabled'
    when p_submission_status = 'hidden' then 'needs_resubmission'
    when p_submission_status in ('submitted', 'published') then 'completed'
    when p_participant_status is not null then 'pending'
    else null
  end;
$$;

revoke all on function public.birthday_wish_action_status(text, text) from public, anon;
grant execute on function public.birthday_wish_action_status(text, text) to authenticated;
