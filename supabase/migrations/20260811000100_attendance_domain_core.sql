...
-- Attendance adjustments are an immutable overlay. Raw QR/manual check-ins remain
-- authoritative in event_attendances and are never rewritten by this migration.
...
comment on table public.attendance_adjustments is
  'Append-only attendance decisions. Original QR/manual check-ins remain in event_attendances.';
...
as $$
  select exists (
    ...
    where event.id = p_event_id
      ...
      and membership.membership_status not in ('invited', 'disabled')
      and (
        membership.membership_status = 'active'
        or (
          event.starts_at <= now()
          and membership.membership_status in ('suspended', 'ended')
          and (membership.membership_status <> 'ended' or membership.ended_on is not null)
        )
      )
  )
$$;
...
create or replace function public.attendance_csv_safe_cell(p_value text)
returns text
language sql
immutable
set search_path = pg_catalog
as $$
  select case
    when p_value is not null
      and left(p_value, 1) in ('=', '+', '-', '@', chr(9), chr(13)) then '''' || p_value
    else p_value
  end
$$;
...
as $$
  ...
  join public.clubs as club on club.club_status = 'active'
...
revoke all on function public.attendance_csv_safe_cell(text) from public, anon, authenticated;
