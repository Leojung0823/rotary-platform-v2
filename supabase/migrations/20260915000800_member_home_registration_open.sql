...
-- 首頁要能對婉拒的社員說「改為報名」。
--
-- registration_state 在社員一有答覆之後就不再提期限——CASE 先看 my_response,
-- 所以 'declined' 同時可能代表「還能改」和「已經來不及」。首頁因此只能給一句
-- 中性的「查看活動」，而那正是社員最需要一條路回去的時候。
--
-- 這裡把「現在還能不能報名」獨立成一個欄位，與答覆無關。判斷式與
-- list_club_events 相同，少掉 published 檢查——能進到這個 projection 的活動
-- 本來就是已發布的。
--
-- 逐字重述目前定義，只多一個欄位；其餘一字未改。
