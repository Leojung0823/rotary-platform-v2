...
-- The home card is the first thing a member sees, and it was the only place an
-- event appeared without its cover: the events list and the event page both
-- show one. Projecting the storage path (never a URL) keeps the signing
-- decision on the server, where the bucket is private and a signed URL has to
-- be minted per request.
--
-- Only the path is added. A member who can see the card could already see the
-- event, so this widens no audience; it hands the page the same string the
-- events list already receives.
