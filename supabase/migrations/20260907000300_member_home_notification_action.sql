...
-- A homepage notification that tells a member to go and do something has to be
-- able to take them there. The message row already carries action_path -- the
-- message centre has rendered it since the collection scheduler shipped -- but
-- the homepage projection dropped it, so "請打開生日祝福徵集完成它" was a dead
-- end that the member had to translate into navigation themselves.
--
-- Nothing else about the projection changes: still one round trip, still only
-- rows delivered to this exact membership, still no message, recipient,
-- account, person or membership identifier. action_path is constrained at the
-- column to a relative in-app path, and the browser re-checks it before it is
-- ever put in an href.
