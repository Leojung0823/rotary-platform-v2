...
-- Turning off "allow birthday wishes" must remove existing wishes from the
-- member-facing V2 wall as well as prevent new submissions. The page copy and
-- the collection wall already promise that behavior; enforce it in the same
-- database projection so a client cannot keep showing stale wishes.
