...
-- 待辦提醒不只有活動報名。
--
-- The column has answered one question since it shipped: which events has this
-- member not replied to. Everything else a member owes the club -- unpaid dues,
-- a birthday wish someone is waiting on, an unread notice, a profile the club
-- cannot reach them through -- lived on a page they had to think to visit.
--
-- pending_tasks becomes a list of several kinds. Each row says what it is, what
-- it is about and where it goes; only the kinds with a real deadline carry one.
-- Two of them deliberately have none:
--
--   * 社費 -- a receivable has no due date of its own in this schema.
--   * 個人資料 -- nothing goes wrong if it is never done.
--
-- A reminder that cannot be late must not be able to shout, so neither gets an
-- hours_remaining and both sort last.
--
-- Restated from the current definition; the events branch is unchanged apart
-- from the fields every kind now carries.
...
begin
  ...
  with candidate_events as (
    ...
    select
      (person.primary_phone is null or btrim(person.primary_phone) = '') as phone_missing,
      ...
      from (
        ...
        select
          ...
          jsonb_build_object(
            ...
            'title', '個人資料未完成',
            'detail', case
              when gaps.phone_missing and gaps.birthday_missing then '缺聯絡電話與生日'
              when gaps.phone_missing then '缺聯絡電話'
              else '缺生日'
            end,
            'action_path', '/me',
