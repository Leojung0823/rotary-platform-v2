...
-- Keep automatic pairing aligned with the member onboarding authorization
-- window. This replaces the deployed function without editing migration
-- history: an active membership must also have started and not ended today.
...
comment on function public.auto_pair_line_oa_follower(uuid, uuid, text) is
  'Trusted exact-match LINE OA follow pairing. Returns a status and never exposes LINE identifiers in audit metadata. Requires a current active membership window.';
