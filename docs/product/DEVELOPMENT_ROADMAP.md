...
- staging 目前 runtime 是 `2f0a9a5bef7e60d4e395b94f4f30ac02ec0c47e3`，`/api/health` 的 `issues` 與 `warnings` 都是空的；`main` 後續多了兩個 E2E 測試提交與本次文件同步提交，均沒有產品程式或 migration。閱讀本文件時仍應以 GitHub `main` 的最新 commit 為權威，不能把尚未部署的測試／文件提交說成 staging 已有。
