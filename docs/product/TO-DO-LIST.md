...
7. **E-05：LINE 推播額度與超額政策** `[>]`：政策已定為「停止並提示」；本機已驗證管理幹部看得到提醒、一般社員看不到，且已補上只允許 staging 測試社團、驗收後清理的 hosted 專項 workflow。待最新 runtime 發布後，以 exact SHA 手動執行 `Staging LINE Quota Notice Acceptance`，確認提示與清理結果。
