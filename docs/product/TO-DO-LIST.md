...
這一節只保留目前仍有效的狀態；更早的 `[>]` 紀錄是歷史接力，不應覆蓋上方最新判定：

- `[x]` 社費、收款與核銷（應收、部分收款、代墊、核銷）已在 PR #108 合併並部署 staging；受保護 hosted 驗收 `35501504704` 已通過，包含回收／調整與權限邊界。
- `[x]` 報表與匯出（社員、活動、出席、財務 CSV／Excel／PDF）已包含在同一 hosted 驗收 `35501504704`；這是 staging 合成資料證據，不代表 production 正式金流。
- `[>]` LINE Rich Menu 與完整 OA 整合已在 PR #107 合併並部署 staging；仍待各社自己的 OA 外部設定與手機驗收（見 E-11）。
- `[>]` 手機 Web App（安裝、離線提示與推播準備）已在 PR #109 合併；仍待真實 iOS／Android 裝置驗收（見 E-07）。
- `[x]` 社務資訊與年度服務計劃已合併至 `main`（PR #111、#124）並包含在目前 staging；hosted 與角色邊界由 `35456938712` 驗收完成。
- `[!]` 社務 AI 助理（摘要、公告草稿、會議紀錄與授權查詢）沒有可執行企劃或已授權的 AI 服務規格；這不是目前 release 的工程待辦，必須先有產品決策才能開工。

### 生日設定 UX 調整（PR #110；目前 hosted 證據已完成）

四項調整都已在 PR #110 實作並進入 staging；生日設定／生日徵集 hosted 驗收 `35503580840` 已成功：

- `[x]` **`/birthdays` 不應該可以切換扶輪社**：頁面沿用全域已選社別。
- `[x]` **生日隱私預設公開**：新社籍明確公開，既有缺列仍維持私密，不會因補生日偷偷公開。
- `[x]` **生日開關整合進 `/me`**：以 `membership_id + club_id` 分開識別，雙重社籍可分社設定。
- `[x]` **重複建立生日徵集時顯示可理解錯誤**：保留每年唯一約束，不放寬資料庫規則。

以上 `[x]` 代表目前已有受控 staging／hosted 證據；不代表已在 production 開啟或建立正式財務資料。
