...
- 本輪已完成程式／verification 檔／單元測試；2026-09-19 本機 Docker 恢復後，`npm run verify:db`、superadmin／role-shell fixture bootstrap、`check:migrations`、`check:db-verifications` 均已通過。現場核對 staging Go-Live `35440209578` 的 runtime 為 `8f109d0fba579397a7f5e8d7d5a591771f09ab2a`；staging `/api/health` 回報 `revision=8f109d0fba57`、`issues=[]`。最新主線為 `b49ae9e8db6337b19058d3033c67bbdfb04b6184`，後續未重新部署 staging。已登入的社務管理頁顯示 25 位社員、15 位已配對、10 位未配對；最新推播為 `sent`，所以目前未顯示額度提醒；不要為了驗收硬打真實額度。本機隔離 fixture 已確認管理幹部看到 rate-limited 提醒與部分送達數字，一般社員被拒絕且看不到提醒；本輪已把 fixture 固定在 `line-oa-audience-1440`，最新一次 `5 passed`。下一步只剩不消耗正式額度的 staging UI 專項證據。

## 2026-09-19 最新現場核對（本節優先）

- 2026-09-19 現場核對 staging runtime exact SHA 為 `8f109d0fba579397a7f5e8d7d5a591771f09ab2a`；Staging Go-Live `35440209578` 成功，staging `/api/health` 回報 `revision=8f109d0fba57`、`status=ok`、`issues=[]`，production 沒有修改。最新主線為 `b49ae9e8db6337b19058d3033c67bbdfb04b6184`；後續只增加本機 fixture、瀏覽器回歸測試與進度文件，未重新部署 staging。自動 CI `35443369737` 與 Browser Smoke `35443369707` 均成功。
...
- Staging runtime 基線仍是 `211b363526ca1398f2cba49708d9c7a0dfca1422`；本輪程式／測試修正為 `16fcc25`、`5f794e1`，目前沒有 open PR。PR #202 已合併，本輪另加入 LINE OA 待辦整合；`main` head 請用 `git rev-parse origin/main` 現場核對。
- `16fcc25`、`5f794e1` 只修正 `member-home` E2E 對舊版 LINE 首頁卡片的過時斷言，沒有改 runtime、資料庫或權限。前一輪 `35310867689` 有 6 個舊斷言失敗；更新一次後 `35312050315` 剩 1 個漏網舊斷言失敗，另有 1 個標籤測試 retry 後通過的 flaky。最新自動 CI `35313186838` 與 Browser Smoke `35313186884` 均已成功，Browser Smoke 的 member-browser-smoke 與 rollback checks 也都成功。
...
## 歷史：最新 GitHub 開發掃描（2026-09-15；#147 與 staging Go-Live）
...
待做：完成一位曾以 LINE Login 登入的社員加入 PANCHIAO OA，確認 follow 事件自動配對到正確姓名，並補測多社、外社、停權／退社不誤配；若 HAPPY 也要有 OA，必須另設 HAPPY 自己的 LINE channel 與 `LINE_OA_HAPPY_*`，不可跨社共用。`birthday-scheduler` 的 GitHub environment 與正確 Render staging service 的秘密值已同步，下一步是準備符合條件的測試收件人，再用 scheduler 實際驗證 LINE 邀請送達與重跑不重送。管理模式生日／文件／活動／封面 hosted acceptance 已完成。舊 workflow 受 `staging` required reviewer 阻擋的問題已由環境隔離修正。舊 webhook row 只保存舊版 raw hash，無法安全回算；不要放寬 payload mismatch 來相容舊資料。
...
**已確認**：使用已登入的 staging 會員切換到 `PANCHIAO-ELITE` 開啟 `/me/line-oa`，頁面已顯示
「加入本社 LINE」連結，且管理頁的 OA 身份驗證與最近 Webhook 簽章均成功。尚未證明真實加入／follow／
自動配對流程。`line_oa_onboarding_v1` 仍有
...
staging health (historical)        status=ok; revision 244ac256c42d;
                                  issues=[]; warnings=[]
```
...
- #123 合併後主線 Browser Smoke `34855905727` 使用同一個 exact SHA，結果為 184 passed、38 skipped、1 failed；
  ...
  但瀏覽器沒有顯示預期文字。#127 的後續 Browser Smoke `34862992487` 已全綠並已合併；這不是 staging Go-Live 失敗。
...
- 合併後 `CI` run `34880910821` 與 `Browser Smoke` run `34880910900` 已針對同一個 merge commit
  成功完成；本輪沒有手動重跑 CI 或 Browser Smoke。
...
## 歷史 staging plan 核對補充（2026-09-15；#137 合併後）
