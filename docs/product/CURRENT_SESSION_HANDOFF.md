...
- 下一步：取得使用者確認後建立／更新 Supabase management token，再以最新 exact SHA 重新 Go-Live；成功後執行社費、生日設定與一般社員越權拒絕的
  hosted acceptance。E-03 真人配對、E-06 登入效能、E-07 實機、E-08 production、E-10 負向矩陣、E-11 Rich Menu 仍不能用本機證據代替。
...
- 本節的產品與 staging 證據對應產品 commit／staging runtime `a4fd0d6f47b38d78922d5bdc3316b4949d191ab6`；後續文件同步只更新紀錄、
  不重新部署，最新 `origin/main` 請以現場 `git rev-parse origin/main` 核對。目前沒有 open PR。既有未追蹤的
