...
## 2026-09-21 E-06 同條件前後比較完成（run `35528969873`）

- 比較方式：同一個 staging、同一套受保護 desktop Chromium 測試、同一個 `warm` 瀏覽器快取條件；修改前 `54c08ef6ab4dca6a75c488226a0bccf1f0b32961` 與修改後 `7253ea009e570948953b796dcd29ea6b79591230` 各頁各取 5 次。測完後 workflow 自動恢復修改後版本，`/api/health` 回報 `revision=7253ea009e57`、`status=ok`、`issues=[]`。
- 結果是同條件的觀測性中位數；負值代表修改後較快，不代表所有裝置、網路或 Render 負載都會得到同樣結果。

| 頁面 | 修改前 LCP／FCP／TTFB／INP | 修改後 LCP／FCP／TTFB／INP | 變化 |
|---|---:|---:|---:|
| 社員首頁 `member-dashboard` | `844／408／388.2／16 ms` | `816／308／283.1／16 ms` | `-3.3%／-24.5%／-27.1%／0.0%` |
| 社務管理首頁 `management-dashboard` | `836／388／371.3／16 ms` | `772／392／368.8／16 ms` | `-7.7%／+1.0%／-0.7%／0.0%` |

- 這次補足了「同一工具／同一 runtime／同一快取條件」的前後比較，E-06 的因果證據項目完成。INP 沒有變化；社務管理頁 FCP 也沒有改善，不能把結果描述成所有指標都變快。
- 測試只保存匿名數字與短暫 CDP trace 摘要（事件數／長任務），不保存原始 trace、畫面、HTML、帳密或頁面內容。
...
## 2026-09-21 cold staging 基線（run `35527405302`）

- 條件與 warm run 相同：產品 runtime `7253ea009e57`、受保護社員／執行秘書身份、desktop Chromium、每頁 5 次；唯一變化是測量頁面前關閉瀏覽器 HTTP cache。

| 頁面 | 第 1 次 | 第 2 次 | 第 3 次 | 第 4 次 | 第 5 次 | 中位數 |
|---|---|---|---|---|---|---|
| 社員首頁 `member-dashboard` | 804／308／280.3／16 | 640／256／238.1／16 | 708／292／269.6／16 | 672／272／249.5／16 | 616／256／223.4／16 | **672／272／249.5／16 ms** |
| 社務管理首頁 `management-dashboard` | 668／268／249.1／16 | 308／308／212.4／16 | 1224／320／296／16 | 736／424／403.4／16 | 240／240／219.1／16 | **668／308／249.1／16 ms** |

> 欄位順序：LCP／FCP／TTFB／INP。CDP trace 摘要：社員 873 事件、6 個長任務、最長 `101.5 ms`；管理 747 事件、5 個長任務、最長 `174.2 ms`；原始 trace 未保存。

- warm／cold 是同一個修改後 runtime 的兩種 cache 條件，不能代替修改前後實驗。它們顯示 cold 條件下仍有長任務，下一步若要找真正瓶頸，應取得同一工具／同一 cache 條件的修改前樣本，再判斷是否需要繼續改查詢或前端 bundle。
...
- 條件：同一 staging runtime、同一受保護社員／執行秘書測試身份、同一 desktop Chromium workflow、每頁 5 次；結果仍是 Playwright browser timing，不是 Chrome DevTools Performance trace。
- 本次結果如下，欄位順序為 LCP／FCP／TTFB／INP：

| 頁面 | 第 1 次 | 第 2 次 | 第 3 次 | 第 4 次 | 第 5 次 | 中位數 |
|---|---|---|---|---|---|---|
| 社員首頁 `member-dashboard` | 1220／484／376.6／16 | 1408／396／378.4／16 | 668／336／316／16 | 608／264／237.4／16 | 776／360／343.3／16 | 776／360／343.3／16 ms |
| 社務管理首頁 `management-dashboard` | 564／248／220.4／16 | 612／300／269.8／24 | 768／304／287.4／16 | 300／300／272.3／16 | 564／252／235.3／16 | 564／300／269.8／16 ms |

- 同一產品 runtime 的前一組 5 次基線是 run `35521567024`：社員 `1132／432／411／16 ms`、管理 `748／364／329.1／16 ms`。兩組方向不同且仍有冷／暖快取與 Render 波動，不能把差異歸因於任何單一修改。
- 結論：本次補強了目前 staging 的可重現觀測，沒有證明效能改善已完成；E-06 仍需要同一快取條件的修改前後比較與登入後 Chrome DevTools trace。沒有新的可靠前後數字前，不再猜測性改查詢或加公開快取。

## 2026-09-21 導覽未讀徽章移出首頁關鍵等待（目前結論：安全部署，尚未證明變快）

- 找到的另一個等待點：登入後外殼在顯示導覽前，會等待未讀訊息數 RPC；這個數字只影響導覽上的徽章，不是登入、角色、權限或首頁內容。
- 已改成：導覽先用 `Suspense` 顯示，未讀數稍後補上；首頁內容不再為了徽章等待。沒有延後任何授權判斷，也沒有把未讀數或個人資料放進公開快取。
- 產品 commit `7253ea009e570948953b796dcd29ea6b79591230` 已推到 `main`，沒有新增 migration、沒有修改 RPC／RLS／登入／社團隔離，也沒有修改 production。
- 本機完整檢查通過：typecheck、lint、Vitest `200` 檔／`1483` tests、build、`verify:db`、migration guard、verification manifest 與 `git diff --check`；lint 只有既有 warning。
- 自動 CI `35520595971`、Browser Smoke `35520595957`、Staging Release `35521243803`、Staging Go-Live `35521341648` 均成功。staging `/api/health` 回報 `revision=7253ea009e57`、`issues=[]`、`warnings=[]`。
- 受保護 staging 效能測試 `35521567024` 在同一 runtime 量測每頁 5 次；結果如下。這是 Playwright 的瀏覽器量測，不是 Chrome DevTools 登入後 trace。

| 頁面 | 第 1 次 | 第 2 次 | 第 3 次 | 第 4 次 | 第 5 次 | 中位數 |
|---|---|---|---|---|---|---|
| 社員首頁 `member-dashboard` | 1444／700／586／16 | 1580／1112／1093.4／16 | 1132／432／411／16 | 852／416／389.1／16 | 732／352／334.2／16 | 1132／432／411／16 ms |
| 社務管理首頁 `management-dashboard` | 1032／696／583.5／16 | 1680／440／329.1／16 | 316／316／297／16 | 336／336／321.3／16 | 748／364／344.2／16 | 748／364／329.1／16 ms |

> 欄位順序：LCP／FCP／TTFB／INP。與舊基準相比，5 次樣本的中位數方向較低，但前後不是同一個 runtime、樣本數也不同，且冷／暖快取與 Render 波動明顯；因此不能把這組數字當成已證明的因果改善。

- 結論：這次改動已安全部署，且縮短了外殼等待未讀徽章的關鍵路徑；E-06 仍不結案。還需要登入後 Chrome DevTools trace，以及更嚴格的同條件前後樣本，才能判定真正瓶頸與改善幅度。
