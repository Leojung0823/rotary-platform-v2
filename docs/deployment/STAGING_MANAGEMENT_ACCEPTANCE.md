...
這份流程補上管理模式企劃書要求的真實驗收：**沒有社員身分的執行秘書**，能從社務總覽找到生日徵集、文件中心與活動管理，並完成可回收的測試操作。
...
只有 workflow 成功，且 summary 顯示 operator path、生日重跑、文件建立／上傳／編輯，以及活動建立／封面／發布／取消都通過，才可把管理模式企劃書對應的 hosted acceptance 標成完成。只有社員驗收通過，不能代替這項執行秘書驗收。
