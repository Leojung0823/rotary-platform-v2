...
本功能提供**社內留言板**：選擇目前有 active 社籍的扶輪社、查看該社最新留言、stable cursor pagination、純文字發布、編輯自己的留言與軟刪除自己的留言。UI 路徑為 `/board`。
...
`Supabase Auth user → active app_accounts → people → active club_memberships`

所有 RPC 從 `auth.uid()` 推導目前 app account，並對傳入的 `club_id` 重新確認 active membership。Browser 不得提供 author、Auth UUID、person ID、email、LINE subject、role 或 ownership flag。
...
Server 與 database 會正規化換行、trim 外圍空白、拒絕空白內容，並限制最多 1000 字。React 以 text node escaping 顯示；未使用 `dangerouslySetInnerHTML` 或 HTML passthrough。

頭像只接受 HTTP(S) URL；無效或載入失敗時顯示姓名首字 fallback。
...
Verification 會 rollback fixture，並驗證：

- `club_id` tenant key 與不可變性
- anonymous denial 與 exact RPC grants
- active membership requirement
- 跨社 list/create/update/cursor denial
- 同社 ownership enforcement
- pagination、projection、constraints、soft delete 與 disabled account denial
...
Vitest 覆蓋 content normalization/limit、unknown fields、UUID、limit、cursor、same-origin、content-type、純文字 rendering、頭像 URL policy、no-store 與 raw-error redaction。資料庫租戶隔離由 verification SQL 驗證。

## 10. UI smoke test

1. 以有 active membership 的帳號登入並開啟 `/board`。
...
本 PR 與 PR #11 共同 base 為 `feat/mvp-line-board-integration`。`src/components/app-shell.tsx` 是 integration-sensitive file；留言板不修改 LINE Login、logout 或 session runtime。

本實作沒有連線或修改 hosted/production Supabase、Lovable、LINE Console、LINE OA、Rich Menu 或 secrets。合併前仍需要 local Supabase verification、browser smoke test 與獨立 reviewer。
