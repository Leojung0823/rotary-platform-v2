...
- **對 provider 的請求有 10 秒逾時**，逾時記為 `provider_timeout`；此時訊息可能已送達，
  重送前要先看推播紀錄。
- **每月推播額度依方案而定。** 達到上限時 LINE 回 429，平台記為 `rate_limited`；產品決定採「停止並提示」：
  同一批安全重試一次後，停止剩餘批次，不繼續消耗額度。手動送出會立即顯示提示；排程／活動推播會保留部分送達紀錄，
  並在社務管理的 LINE OA 頁面通知有 `oa.read` 權限的幹部。下一次推播有其他結果後，舊提醒不再顯示。
