...
> 狀態：**生日 V2 核心、祝福徵集與生日邀請 LINE 推播已部署並通過 staging Go-Live；最新 scheduler 執行成功但沒有符合條件的 LINE 收件人，生日邀請實際 LINE 送達與 M1 真人使用者測試待完成**
> 程式現況基準：staging 產品 runtime 為 `fbdc061dd702`，本次文件同步只會新增文件 commit，不會改變產品 runtime。生日相關程式、migration、旗標與 scheduler secret 已完成 staging 設定。
...
徵集依賴 V2 核心、已存在但預設關閉的**站內訊息中心**，以及 staging-only 的 GitHub Actions 排程入口；第一至三階段程式、migration、staging Go-Live 與 hosted acceptance 已完成。staging 的兩個生日旗標與 Render scheduler secret 已設定且已同步；最新 scheduler `34673612440` 執行成功，但 `line_push.jobCount=0`、`sentCount=0`，因為本次沒有符合條件的收件人，所以每日排程與本輪生日邀請 LINE 實際送達仍未證明。M1 真人社員／幹部測試仍未完成。
...
排程環境問題已查明並修正：舊 workflow job 使用需要 required reviewer 的 `staging` environment，現在改用只允許 `main` 的 `birthday-scheduler` environment。GitHub 與正確 Render staging service 的 scheduler secret 已同步；最新 run `34673612440` 已成功到達 protected staging route，但因沒有符合條件的 LINE 收件人而回報 `jobCount=0`、`sentCount=0`。下一步是準備已配對、仍追蹤且開啟通知的測試社員，再驗證實際收件與冪等；不能移除 staging 部署保護。
...
本機資料庫驗證：同一批次重跑只保留一筆通知；訊息中心旗標關閉時保留 `skipped` retry marker，重新開啟後補送原批次，不建立第二個任務或第二則訊息；每位收件社員的訊息會顯示 `pending`、`completed`、`declined` 或 `needs_resubmission`。staging hosted acceptance 已驗證生日 V2 與徵集入口；最新 scheduler `34673612440` 已成功呼叫受保護的 staging route，但因沒有符合條件的 LINE 收件人而回報 `jobCount=0`、`sentCount=0`。這表示路由與排程可運作，仍需有收件人的實際 LINE 送達驗收。
...
- `.github/workflows/birthday-collection-scheduler.yml` 與 `/api/internal/birthday-collection/scheduler` 已推送並只接 staging；環境隔離修正已部署到 `main`。GitHub `birthday-scheduler` 與正確 Render staging service 的 secret 已同步，最新 run `34673612440` 成功但 `jobCount=0`、`sentCount=0`；要用有收件人的測試資料驗證實際 LINE 送達與重跑不重送，不能用移除 staging 保護的方式修復。
...
第一至三階段的程式開發與本機安全驗證已完成；PR #77 已完成 main 整合與 staging 發布，兩個生日 flag 已開啟，Render／GitHub scheduler secret 已同步，GitHub scheduler workflow 已改用獨立 environment。本輪 LINE 邀請推播也已合併並部署，尚未完成的是有收件人的 scheduler 營運實跑、LINE 實際送達驗收與 M1 真人社員／幹部使用者測試。
...
- `20260824000600` 至 `20260824001700` 已在目前 `main`；PR #77 已完成 main 整合，且生日相關 staging migration、部署與 hosted acceptance 已完成。最新 scheduler `34673612440` 成功但沒有符合條件的 LINE 收件人；production 不在本輪範圍。
...
- 已測月批重跑冪等、題庫不足時整批暫停且補題後同批次重試、同批次題目 ID／文字不重複、跨社團隔離、發布權限、feature flag server gate 與 DB `authenticated` EXECUTE gate、排程重跑、訊息重複派發冪等、題庫管理權限、婉拒、隱藏重送、append-only 處理紀錄、逐位訊息狀態，以及 `allow_wishes=false` 的核心投影；生日 staging Go-Live 與 hosted acceptance 已通過。最新 scheduler `34673612440` 成功但 `jobCount=0`、`sentCount=0`；仍未完成的是有收件人的 LINE 實際送達與 M1 真人社員／幹部驗收。
