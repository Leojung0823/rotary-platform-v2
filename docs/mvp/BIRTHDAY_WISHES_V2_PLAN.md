...
> 更新日期：2026-08-24
> 狀態：**生日 V2 核心與祝福徵集第一至三階段已在本機實作，尚未發布**
> 程式現況基準：`main` @ `5d0d100`；`20260824000700`–`20260824001500`、UI、排程 route、workflow 與 verification 仍是未提交修改
...
徵集依賴 V2 核心、已存在但預設關閉的**站內訊息中心**，以及 staging-only 的 GitHub Actions 排程入口；目前都只在本機驗證，尚未發布。
...
本機已驗證：同一批次重跑只保留一筆通知；訊息中心旗標關閉時保留 `skipped` retry marker，重新開啟後補送原批次，不建立第二個任務或第二則訊息；每位收件社員的訊息會顯示 `pending`、`completed`、`declined` 或 `needs_resubmission`。尚未在 staging 執行 workflow。
...
第一至三階段的本機開發與安全驗證已完成；尚未完成的是 staging flag、發布部署、真人社員驗收與 M1 使用者測試。
...
- `20260824000600` 已在目前 `main`；`20260824000700`–`20260824001600` 仍是本機未提交的 forward-only migration，不能直接視為已部署。
...
- 已測月批重跑冪等、題庫不足時整批暫停且補題後同批次重試、同批次題目不重複、跨社團隔離、發布權限、feature flag server gate 與 DB `authenticated` EXECUTE gate、排程重跑、訊息重複派發冪等、題庫管理權限、婉拒、隱藏重送、append-only 處理紀錄、逐位訊息狀態，以及 `allow_wishes=false` 的核心投影；尚未做 hosted workflow 與社員驗收。
