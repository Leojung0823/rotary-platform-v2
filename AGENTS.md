...
`docs/development/COMMON_PITFALLS.md` 收錄了這個 repo 真的發生過、而且**當下所有本機檢查都是綠的**那些錯誤：函式重述的基礎、空洞的守則、SQL 到 CI 才會解析、分眾規則要在每條路徑重複套用、`gh pr checks` 的聚合、migration 亂序、**PR 有衝突時 CI 根本不會跑**、grid 版面的 `min-width: 0`、被當死碼刪掉的 CSS 類名、**改一個可見字串會打斷把它當選擇器的 e2e**、**`->>` 的 null 混進 `||` 會讓整頁消失**、**提醒要能被它指過去的那一頁清掉** 等。寫新守則或改可見文字前先讀一次。
