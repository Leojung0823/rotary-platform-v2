...
serveInvitationHandler(async (request) => {
  ...
  requireOnlyKeys(input, ["membership_id", "idempotency_key", "expires_in_seconds"])
  const membershipId = requireUuid(input.membership_id)
  ...
  const result = await callInvitationRpc("create_membership_invitation", {
    ...
    p_token_issued_at: material.payload.issued_at
      ? new Date(material.payload.issued_at * 1000).toISOString()
      : issuedAt.toISOString(),
    p_expires_at: new Date(material.payload.expires_at * 1000).toISOString(),
    p_delivery_channel: "manual_link",
    p_destination_masked: null,
